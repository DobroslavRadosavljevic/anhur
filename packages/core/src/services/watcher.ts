...
>()("@anhur/core/Watcher") {
  ...
  > {
    return createWatcherLayer();
  }
}

function createWatcherLayer(): Layer.Layer<
  Watcher,
  never,
  Builder | ConfigLoader | FileSystem.FileSystem
> {
  return Layer.effect(
    Watcher,
    Effect.fn("makeWatcher")(function* () {
      ...
      const start = Effect.fn("start")(function* (
        ...
        const watchExtraSourcePaths = Effect.fn("watchExtraSourcePaths")(
        ...
        const watchPaths = loaded
          ...
          : [
              ...
              canonicalizePath(path.join(path.dirname(absoluteConfig), "cms")),
            ];
        ...
        return {
          close: async () => {
            await runEffectPromise(Fiber.interruptAll([fiber, ...extraFibers]));
          },
        };
      });
